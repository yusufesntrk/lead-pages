...
## Döner Build Example with React Spring

```jsx
function DonerBuilder() {
  const ref = useRef();

  return (
    <Parallax pages={7} ref={ref}>

      {/* Deep Background - Kitchen gradient */}
      <ParallaxLayer
        offset={0}
        speed={0.1}
        factor={7}
        style={{
          background: 'radial-gradient(circle at 50% 50%, #FF9500 0%, #8B4513 100%)',
        }}
      />

      {/* Döner Spieß - Rotating spit */}
      <ParallaxLayer
        offset={0.5}
        speed={0.3}
        style={{ display: 'flex', justifyContent: 'center' }}
      >
        <img src="/spit.png" className="rotating-spit" />
      </ParallaxLayer>

      {/* Step 1: Bread opens (0-15%) */}
      <ParallaxLayer
        offset={0}
        speed={0.8}
        style={{ display: 'flex', justifyContent: 'center' }}
      >
        <img src="/bread-open.png" className="fade-in" />
      </ParallaxLayer>

      {/* Step 2: Meat flies in (15-30%) */}
      <ParallaxLayer
        offset={1}
        speed={1.5}
        style={{ display: 'flex', justifyContent: 'flex-end' }}
      >
        <img src="/meat.png" className="slide-rotate-in" />
      </ParallaxLayer>

      {/* Step 3: Lettuce from left (30-45%) */}
      <ParallaxLayer
        offset={2}
        speed={1.2}
        style={{ display: 'flex', justifyContent: 'flex-start' }}
      >
        <img src="/lettuce.png" className="slide-left" />
      </ParallaxLayer>

      {/* Step 4: Tomatoes zoom (45-60%) */}
      <ParallaxLayer
        offset={3}
        speed={1.8}
        style={{ display: 'flex', justifyContent: 'center' }}
      >
        <img src="/tomato.png" className="zoom-in" />
      </ParallaxLayer>

      {/* Step 5: Onion rings fall (60-75%) */}
      <ParallaxLayer
        offset={4}
        speed={2}
        style={{ display: 'flex', justifyContent: 'center', alignItems: 'flex-start' }}
      >
        <img src="/onions.png" className="fall-down" />
      </ParallaxLayer>

      {/* Step 6: Sauce drips (75-100%) */}
      <ParallaxLayer
        offset={5}
        speed={1.5}
        style={{ display: 'flex', justifyContent: 'center' }}
      >
        <img src="/sauce.png" className="drip-animation" />
      </ParallaxLayer>

      {/* Final assembled Döner - Sticky */}
      <ParallaxLayer
        sticky={{ start: 5.5, end: 6.5 }}
        style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}
      >
        <img src="/doner-complete.png" className="pulse" />
      </ParallaxLayer>

      {/* Spice Particles - Foreground */}
      <ParallaxLayer offset={0} speed={2.5} factor={7}>
          ...
          {[...Array(30)].map((_, i) => (
            <div
              ...
              style={{
                ...
                backgroundColor: ['#FFD700', '#FFA500', '#FF4500', '#32CD32'][i % 4]
              }}
            />
          ))}
        </div>
...
/* Slide and rotate in */
.slide-rotate-in {
  animation: slideRotate 1s ease-out;
}
...
ref.current.scrollTo(2); // Go to page 3 (0-indexed)
